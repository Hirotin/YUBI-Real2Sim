...
The OBS scores shown in this section are descriptive scores on the right/left OBS cameras. They are not the OBS inputs used in Table S1. The CrossScore / OBS-TableII row in Table S1 is instead evaluated on the observation images used for the paper's NVS-SQA / OBS results.
...
The variant Test scores are the Table II inputs. These NVS-SQA scores use all Test views, not the person-free subset of Section 3; NVS-SQA scores an image set as a whole, so a subset value would require recomputation.
...
- For the five quality-based methods of Table II and for the constant predictors, nothing is refit. These numbers are recomputed from the out-of-fold prediction log behind Table II: the same conditions, the same fitted thresholds, the same predictions.
- The CrossScore rows (marked †) are new. Their thresholds are fitted here with the same protocol as Table II (scene-held-out, balanced-accuracy threshold, single-class fallback) on the same 15 conditions and labels. The re-implemented classifier reproduces all 300 logged Table II predictions when run on Table II's own inputs.
